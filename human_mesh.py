import os, pickle, torch, smplx, trimesh, colorsys, tempfile, gc, random
import numpy as np
from scipy.spatial.transform import Rotation as R
import pybullet as p
from .agent import Agent
from view_fits import get_smpl
...
class HumanMesh(Agent):
    ...
    def use_smplx_body(pkl):
        # Pick a random pkl file
        ## pick a random person 1-102 (pn)
        ...
        #Load in using 3DFits Code
        model = smplx.create('models', model_type='smpl', gender=pkl_fit['gender'])

        output = model(betas=torch.Tensor(pkl_fit['betas']).unsqueeze(0),
                   body_pose=torch.Tensor(pkl_fit['body_pose']).unsqueeze(0),
                   transl=torch.Tensor(pkl_fit['transl']).unsqueeze(0),
                   global_orient=torch.Tensor(pkl_fit['global_orient']).unsqueeze(0),
                   return_verts=True)
        
        #everything above this line should run!
        orientation=[0,0,0]
        id=0
        #below is code from create_smplx_body
        vertices = output.vertices.detach().cpu().numpy().squeeze()
        joints = output.joints.detach().cpu().numpy().squeeze()
        # Scale vertices and rotate
        # orientation is where [0,0,0] was
